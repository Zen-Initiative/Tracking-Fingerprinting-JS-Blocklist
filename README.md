...
The strict version of the uBO blocklists makes use of the `$all` option in [uBO's extended syntax](https://github.com/gorhill/uBlock/wiki/Static-filter-syntax#all) to protect `inline script tags` and `inline font tags` (which can be used for fingerprinting) when another filter list contains an exception to unblock the URL.  
